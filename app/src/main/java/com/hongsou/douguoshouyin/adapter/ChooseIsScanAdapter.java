...
public class ChooseIsScanAdapter extends BaseQuickAdapter<SingleFoodsBean, BaseViewHolder> {
    public ChooseIsScanAdapter(int layoutResId, @Nullable List<SingleFoodsBean> data) {
    ...
    protected void convert(BaseViewHolder helper, SingleFoodsBean item) {
        ...
        helper.setText(R.id.tv_item_choose_isscan_name,item.getSingleProductName())
                .setText(R.id.tv_item_choose_isscan_price,item.getSingleProductPrice());

        if ("1".equals(item.getIsScan())){
            helper.setImageResource(R.id.iv_item_choose_isscan_choose,R.drawable.checkbox);
        }else {
            helper.setImageResource(R.id.iv_item_choose_isscan_choose,R.drawable.checkboxn);
